...
st.sidebar.markdown("Sube los archivos correspondientes al período de evaluación.")

archivo_gen = st.sidebar.file_uploader("1. Archivo de Generación Real (Ucuquer)", type=['xlsx', 'csv'])
archivo_cmg = st.sidebar.file_uploader("2. Archivo de Costos Marginales", type=['xlsx', 'csv'])

if archivo_gen is not None:
    try:
        df_completo = cargar_y_procesar_datos(archivo_gen)
        hay_datos_cmg = False
        dias_periodo = len(df_completo['Fecha'].unique())
        
        if archivo_cmg is not None:
            try:
                df_precios = cargar_cmg(archivo_cmg)
                ...
                st.sidebar.error(f"Error procesando CMg: {e}")
        ...
        reporte_txt = f"""REPORTE EJECUTIVO DE ACTIVOS - COMPLEJO UCUQUER
================================================
...
"""
  ...
  - Máxima Inyección Horaria: {formato_cl(row['Max_Generacion'])} MW\n"""
        ...
        st.sidebar.download_button("📄 Descargar Reporte Técnico (TXT)", data=reporte_txt, file_name="Reporte_Activos_Ucuquer.txt", mime="text/plain")
        
        csv_data = df_completo.to_csv(index=False, sep=';', decimal=',')
        st.sidebar.download_button("📊 Descargar Base de Datos Cruzada (CSV)", data=csv_data, file_name="Data_Ucuquer_Consolidada.csv", mime="text/csv")

    except Exception as e:
        st.error(f"Error procesando la plataforma: {e}")
else:
    st.info("👈 Por favor, sube el archivo de Generación Real en la barra lateral para comenzar el análisis.")